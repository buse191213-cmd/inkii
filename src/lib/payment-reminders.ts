...
/**
 * Kernlogik: prüft offene Online-Zahlungen und verschickt 1./2. Erinnerung.
 * Wird sowohl vom Cron-Endpoint als auch beim Öffnen des Admin-Panels
 * (best-effort, nicht blockierend) aufgerufen. Durch den paymentReminders-
 * Zähler werden Doppel-Mails verhindert — egal wie oft die Funktion läuft.
 */
export async function runPaymentReminders(): Promise<{ ok: boolean; sent1: number; sent2: number; checked: number; error?: string }> {
  if (!isSmtpConfigured()) return { ok: false, sent1: 0, sent2: 0, checked: 0, error: "SMTP nicht konfiguriert" };
  ...
  try {
    ...
    for (const order of pending) {
      ...
      // 2. Erinnerung: älter als 24h, noch keine 2. gesendet
      ...
      // 1. Erinnerung: älter als 1h, noch keine gesendet
    ...
    return { ok: true, sent1, sent2, checked: pending.length };
  } catch (e) {
    console.error("[payment-reminders] error:", e);
    return { ok: false, sent1, sent2, checked: 0, error: e instanceof Error ? e.message : "Fehler" };
  }
}
