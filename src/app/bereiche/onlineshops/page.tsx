...
export const metadata: Metadata = {
  title: "Onlineshops für Unternehmen | INKII Works",
  alternates: { canonical: "/bereiche/onlineshops" },
};

export default async function Page() {
  const locale = await getLocale();
  const d = getDictionary(locale);
  const t = d.onlineshopsSub;
  const heroImg = await getHomeImage("os-hero");
  const imgs = [
    await getHomeImage("os-1"),
    await getHomeImage("os-2"),
    await getHomeImage("os-3"),
    await getHomeImage("os-4"),
    await getHomeImage("os-5"),
  ];
  return (
    <SiteShell>
