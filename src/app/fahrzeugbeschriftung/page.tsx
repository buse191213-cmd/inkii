...
export const metadata: Metadata = {
  title: "Fahrzeugbeschriftung & Außenwerbung | INKII Works",
  description: "Fahrzeugbeschriftung, Vollverklebung, Schaufensterbeschriftung und Außenwerbung — von der Folie bis zur Komplettmontage.",
...
export default async function FahrzeugPage() {
  ...
  const t = d.fahrzeug;
  const heroImg = await getHomeImage("fb-hero");
  const imgs = [
    await getHomeImage("fb-1"),
    await getHomeImage("fb-2"),
    await getHomeImage("fb-3"),
    await getHomeImage("fb-4"),
    await getHomeImage("fb-5"),
  ];
